...
// ONLY for ADMIN users
 function dnldPDFBtnHdlr() {
    var emptyUsrMsg = ( _sessLang == SESS_LANG_CHN ) ? '請選擇申請人！' : 'Please select requestors!';
    var emptyPwMsg = ( _sessLang == SESS_LANG_CHN ) ? '請選擇牌位！' : 'Please select name plaque type!';
    var emptyPaperSizeMsg = ( _sessLang == SESS_LANG_CHN ) ? '請選擇 PDF 紙張尺寸！' : 'Please select PDF paper size!';
    ...
    if($("select[name=pdfPaperSize]").val().length == 0) {
        alert(emptyPaperSizeMsg);
        return;
    }
} // dnldPDFBtnHdlr()